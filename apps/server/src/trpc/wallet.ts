...
export const walletRouter = router({
  ...
  sendToken: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Get user's wallet
      const wallet = await WalletService.getUserWallet(user.sub);
      ...
      console.log(`  From User ID: ${user.sub}`);
  ...
  createWallet: privateProcedure.mutation(async ({ ctx }) => {
    ...
    const wallet = await WalletService.createWalletForUser(user.sub);
  ...
  getETHBalance: privateProcedure.query(async ({ ctx }) => {
    const { user } = ctx;

    const balance = await WalletService.getUserETHBalance(user.sub);
  ...
  getTokenBalance: privateProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const balance = await WalletService.getUserTokenBalance(user.sub, input.tokenAddress);
