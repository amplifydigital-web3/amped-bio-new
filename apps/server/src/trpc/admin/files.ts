...
export const filesRouter = router({
  ...
  getFileDetails: adminProcedure.input(FileActionSchema).query(async ({ input }) => {
    const { fileId } = input;
  ...
  getFileDownloadUrl: adminProcedure.input(FileActionSchema).query(async ({ input }) => {
    const { fileId } = input;
  ...
  getFilePreviewUrl: adminProcedure.input(FileActionSchema).query(async ({ input }) => {
    const { fileId } = input;
  ...
  deleteFile: adminProcedure.input(FileActionSchema).mutation(async ({ input }) => {
    const { fileId } = input;
    ...
    try {
      ...
      if (fileUsages.themesWithBackgrounds.length > 0) {
        const themes = fileUsages.themesWithBackgrounds
          .map(t => {
            const themeName = t.name || `Theme #${t.id}`;
            const userName = t.user?.name ? ` (by ${t.user.name})` : "";
            return `${themeName}${userName}`;
