...
export function NdauConnect({
  ...
  const socketUrl = process.env.NEXT_PUBLIC_NDAU_SOCKET_URL;
  ...
  useEffect(() => {
    if (!socketUrl) {
      setConnectionError(
        "NDAU account socket URL not configured. Please set NEXT_PUBLIC_NDAU_SOCKET_URL environment variable."
      );
