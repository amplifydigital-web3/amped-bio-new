...
export class Web3Auth extends Web3AuthNoModal implements IWeb3AuthModal {
  ...
  public async init(options?: { signal?: AbortSignal }): Promise<void> {
    ...
    try {
      ...
      // init login modal
      const { filteredWalletRegistry, disabledExternalWallets } = this.filterWalletRegistry(
        walletRegistry,
        projectConfig
      );
      ...
      trackData = {
        ...
        auth_ux_mode:
          authConnector?.authInstance?.options?.uxMode || this.coreOptions.uiConfig?.uxMode,
      };
  ...
  public async connect(): Promise<IProvider | null> {
    if (!this.loginModal)
      throw WalletInitializationError.notReady("Login modal is not initialized");
    // if already connected return provider
    if (this.connectedConnectorName && this.status === CONNECTOR_STATUS.CONNECTED && this.provider)
  ...
  protected initUIConfig(projectConfig: ProjectConfig) {
    ...
    if (!this.options.uiConfig.defaultLanguage)
      this.options.uiConfig.defaultLanguage = getUserLanguage(
        this.options.uiConfig.defaultLanguage
      );
    ...
    const defaultAuthConnections = projectConfig.embeddedWalletAuth
      .filter(x => x.isDefault)
      .map(x => x.authConnection);
  ...
  protected getInitializationTrackData(): Record<string, unknown> {
    return {
      ...
      modal_auth_connector_login_methods: Object.keys(
        this.modalConfig?.connectors?.[WALLET_CONNECTORS.AUTH]?.loginMethods || {}
      ),
      // UI config
      ...
      ui_modal_display_installed_external_wallets:
        this.options.uiConfig?.displayInstalledExternalWallets,
  ...
  private filterWalletRegistry(
    ...
    const { disableAllRecommendedWallets, disableAllOtherWallets, disabledWallets } =
  ...
  private async initConnectors({
    ...
    const { hasInAppConnectors, hasExternalConnectors } =
      await this.checkConnectorAvailability(filteredConnectorNames);
    const filteredConnectors = connectors.filter(x =>
      filteredConnectorNames.includes(x.name as WALLET_CONNECTOR_TYPE)
    );
      ...
      if (hasInAppConnectors) {
        ...
        this.initExternalConnectors(
          filteredConnectors.filter(
            x =>
              x.type === CONNECTOR_CATEGORY.EXTERNAL &&
              x.name !== WALLET_CONNECTORS.WALLET_CONNECT_V2
          ),
          {
  ...
  private async filterConnectors({
    ...
    const embedWalletConfigMap: Map<string, AuthConnectionConfigItem & { isDefault?: boolean }> =
      new Map();
    for (const authConnectionConfig of projectConfig.embeddedWalletAuth || []) {
      const { isDefault, authConnection, groupedAuthConnectionId, authConnectionId } =
    ...
    Object.keys(this.modalConfig.connectors[WALLET_CONNECTORS.AUTH].loginMethods).forEach(key => {
      ...
      if (groupedAuthConnectionId || authConnectionId) {
        ...
        const configFromDashboard = embedWalletConfigMap.get(
    ...
    this.modalConfig.connectors = deepmerge(
      dashboardConnectorConfig,
      cloneDeep(this.modalConfig.connectors || {})
    );
    ...
    const connectorNames = allConnectorNames.map((connectorName: WALLET_CONNECTOR_TYPE) => {
      ...
      const defaultConnectorConfig = {
        label:
          ...
          connectorName.split("-").map(capitalizeFirstLetter).join(" "),
        showOnModal: true,
      };
      ...
      log.debug(
        "connector config",
        connectorName,
        this.modalConfig.connectors?.[connectorName]?.showOnModal,
        connector
      );
      ...
      if (!connector) {
        if (connectorConfig.showOnModal)
          throw WalletInitializationError.invalidParams(
            `Connector ${connectorName} is not configured`
          );
        return;
      }
      ...
      if (
        ...
        connector.name !== WALLET_CONNECTORS.METAMASK
      ) {
  ...
  private async checkConnectorAvailability(
    ...
    const hasInAppConnectors = this.connectors.some(connector => {
      ...
      if (
        this.modalConfig.connectors?.[connector.name as WALLET_CONNECTOR_TYPE]?.showOnModal !== true
      )
        return false;
      if (!this.modalConfig.connectors?.[connector.name as WALLET_CONNECTOR_TYPE]?.loginMethods)
        return true;
      if (
        Object.values(
          this.modalConfig.connectors[connector.name as WALLET_CONNECTOR_TYPE].loginMethods
        ).some(method => method.showOnModal)
      )
        return true;
      return false;
    });
  ...
  private async initExternalConnectors(
    ...
    externalConnectors.forEach(async connector => {
      ...
      if (!connectorChainNamespaceMap[connectorName])
        connectorChainNamespaceMap[connectorName] = new Set();
      if (connector.connectorNamespace === CONNECTOR_NAMESPACES.MULTICHAIN) {
        ...
        connectorChainNamespaceMap[connectorName].add(
          connector.connectorNamespace as ChainNamespaceType
        );
      }
  ...
  private onInitExternalWallets = async (params: {
    externalWalletsInitialized: boolean;
  }): Promise<void> => {
  ...
  private onSocialLogin = async (params: {
  ...
  private onExternalWalletLogin = async (params: {
    ...
    try {
      const connector = this.getConnector(
        params.connector as WALLET_CONNECTOR_TYPE,
        params.loginParams?.chainNamespace
      );
      ...
      const shouldStartConnectionInBackground =
        connector.name === WALLET_CONNECTORS.WALLET_CONNECT_V2 ||
        (connector.name === WALLET_CONNECTORS.METAMASK && !connector.isInjected);
  ...
  private onModalVisibility = async (visibility: boolean): Promise<void> => {
    ...
    if (wcConnector) {
      ...
      if (
        visibility &&
        (walletConnectStatus === CONNECTOR_STATUS.READY ||
          walletConnectStatus === CONNECTOR_STATUS.CONNECTING)
      ) {
