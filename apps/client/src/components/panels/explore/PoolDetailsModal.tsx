...
import { X, Trophy, Clock, Users, Coins, TrendingUp, Target, Calendar, ExternalLink, Plus, Minus, Info, Edit3, Save, XCircle, Trash2, Share2 } from 'lucide-react';
...
export default function PoolDetailsModal({ isOpen, onClose, pool }: PoolDetailsModalProps) {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(poolUrl).then(() => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'staking': return 'bg-blue-500 text-white';
      case 'social': return 'bg-green-500 text-white';
      case 'trading': return 'bg-purple-500 text-white';
      case 'community': return 'bg-orange-500 text-white';
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'ending-soon': return 'bg-yellow-500 text-white';
  ...
  const handleUtilityChange = (tierIndex: number, utilityIndex: number, field: string, value: string) => {
    const updatedTiers = [...editedTiers];
    updatedTiers[tierIndex].utilities[utilityIndex][field] = value;
    setEditedTiers(updatedTiers);
  };

  const daysRemaining = Math.ceil((new Date(pool.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                {(isEditingTiers ? editedTiers : stakingTiers).map((tier) => {
                  ...
                  const isCurrent = userStake >= tier.minStake && 
                    (stakingTiers[tier.level] ? userStake < stakingTiers[tier.level].minStake : true);
                            ...
                            {isEditingTiers ? (
                              <div className="space-y-2">
                                <input
                                  type="text"
                                  value={tier.name}
                                  onChange={(e) => handleTierChange(tier.level - 1, 'name', e.target.value)}
                                ...
                                <div className="flex items-center space-x-2">
                                  <input
                                    type="number"
                                    value={tier.minStake}
                                    onChange={(e) => handleTierChange(tier.level - 1, 'minStake', parseInt(e.target.value))}
                                  ...
                                  />
                                  <span className="text-xs text-gray-600">+ {pool.stakeCurrency}</span>
                                </div>
                              </div>
                            ) : (
                              <div>
                            ...
                            {isEditingTiers ? (
                              <div className="flex items-center space-x-2 flex-1">
                                <input
                                  type="text"
                                  value={utility.icon}
                                  onChange={(e) => handleUtilityChange(tier.level - 1, utilityIndex, 'icon', e.target.value)}
                                ...
                                <input
                                  type="text"
                                  value={utility.label}
                                  onChange={(e) => handleUtilityChange(tier.level - 1, utilityIndex, 'label', e.target.value)}
              ...
              {!isEditingTiers && (
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-sm text-blue-800">
                    💡 <strong>Your current stake:</strong> {pool.stakedAmount.toLocaleString()} {pool.stakeCurrency}
                    {pool.stakedAmount < 25000 && (
                      <span className="block mt-1">
                        Stake {(stakingTiers.find(t => pool.stakedAmount < t.minStake)?.minStake || 25000) - pool.stakedAmount} more to unlock the next tier!
                      </span>
                    )}
                  </p>
                </div>
