...
export const downloadMediaFromUrl = async (
  ...
  try {
    ...
    if (!mediaType) {
      ...
      if (urlLower.includes('.mp4') || urlLower.includes('.mov') || urlLower.includes('.webm') || urlLower.includes('.avi')) {
        mediaType = 'video';
      } else {
        mediaType = 'image';
      }
    ...
    if (mediaType === 'video') {
      ...
      for (const strategy of corsStrategies) {
        ...
        try {
          ...
          console.log(`Fetch response status with ${strategy.description}:`, response.status, response.statusText);
          console.log('Response headers:', Object.fromEntries(response.headers.entries()));
          
          if (!response.ok) {
            console.warn(`Failed with ${strategy.description}: ${response.status} ${response.statusText}`);
          ...
          console.log(`Successfully created file with ${strategy.description}:`, { name: file.name, size: file.size, type: file.type });
      ...
      throw new Error('Failed to download media with all CORS strategies. The resource may not be accessible due to CORS restrictions or the server may be unreachable.');
    }
  } catch (error) {
