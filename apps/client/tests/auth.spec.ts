...
test.describe("Authentication", () => {
  ...
  test("should display validation errors for empty fields", async ({ page }) => {
    ...
    await expect(
      page
        ...
        .locator("..")
        .filter({ hasText: "Password must be at least 6 characters long" })
    ).toBeVisible();
  });
  ...
  test.describe("Authentication with real API", () => {
    ...
    test.beforeAll(() => {
      ...
      console.log(
        `Using test credentials - Email: ${testUser.email}, Onelink: ${testUser.onelink}, Password: ${testUser.password}`
      );
    });
    ...
    test("should handle successful registration and redirect to dashboard", async ({ page }) => {
      ...
      console.log(
        `Submitting registration with: ${testUser.email}, ${testUser.onelink} ${testUser.password}`
      );
      ...
      const sidebar = page
        .locator("div")
        .filter({ hasText: /Home\s*Profile\s*Themes\s*Appearance\s*Effects\s*Blocks/ })
        .first();
    ...
    test("should handle login errors", async ({ page }) => {
      ...
      await expect(
        page.locator(".bg-red-50").filter({
          hasText: /Invalid credentials|Invalid email or password|Authentication failed/,
        })
      ).toBeVisible({ timeout: 5000 });
