...
interface InstagramEmbedProps {
    /**
     * The URL of the Instagram post to embed
     */
    url: string;
    /**
     * Width of the embedded post (in pixels)
     */
    width?: number;
    /**
     * Hide caption of the Instagram post
     */
    hideCaption?: boolean;
    /**
...
/**
 * A component to embed Instagram posts from a URL
 */
const InstagramEmbed: React.FC<InstagramEmbedProps> = ({
    ...
    useEffect(() => {
        ...
        if (!isValidInstagramUrl(url)) {
            setError('Invalid Instagram URL format. URL should contain instagram.com/p/ or instagram.com/reel/');
            return;
        }
        ...
        if (!existingScript) {
            ...
            script.id = 'instagram-embed-script'; // Add an ID to identify this script
            script.async = true;
            script.defer = true;
            ...
            console.log('Instagram script already exists');
        ...
        // No cleanup of the script on unmount - we'll keep it for future embeds
        // Only clean up event handlers or other component-specific resources
        return () => {
    ...
    if (error) {
        return <div className="instagram-embed-error">{error}</div>;
    }

    // We don't need to construct embed URL manually anymore
    // as we're using the Instagram blockquote + script approach

    return (
        <div className={`instagram-embed-container ${className}`} style={{ maxWidth: width }}>
            {!isLoaded && <div className="instagram-embed-loading">Loading Instagram post...</div>}
            <blockquote
                className="instagram-media"
                data-instgrm-permalink={url}
                data-instgrm-version="14"
                data-instgrm-captioned={hideCaption ? '' : 'true'}
                data-width={'100%'}
                style={{
                    background: '#FFF',
                    border: '0',
...
// Add window type definition for Instagram embed
declare global {
    interface Window {
        instgrm?: {
            Embeds: {
